...
NUMBER_OF_EPISODES_PER_BATCH = 100
...
class DiscreteObservationOneHotEncoder(gym.ObservationWrapper):
    def __init__(self, env):
        super().__init__(env)
        assert(isinstance(env.observation_space, gym.spaces.Discrete))
        shape = (env.observation_space.n,) #n is a property only of Discrete spaces
        self.observation_space = gym.spaces.Box(0.0, 1.0, shape, dtype=np.float32)
    def observation(self, observation):
        position_vector = np.copy(self.observation_space.low) 
        position_vector[observation] = 1.0
...
def filter_batch(batch: List[Episode], device: str) -> Tuple[torch.FloatTensor, torch.LongTensor, float, float]:
    ...
    rewards = [episode.total_reward*gamma**(len(episode.steps)-1) for episode in batch]
    ...
    return best_episodes, torch.FloatTensor(best_observations).to(device), torch.LongTensor(best_actions).to(device), minimum_reward, rewards_mean

if __name__ == "__main__":
    device = 'cuda:1'
    env = gym.make('FrozenLake-v1')
    env = DiscreteObservationOneHotEncoder(env)
    ...
    for iteration_number, batch in enumerate(iterate_batches(env, net, NUMBER_OF_EPISODES_PER_BATCH)):
        best_episodes, best_observations, best_actions, minimum_reward, mean_reward = filter_batch(batch + best_episodes, device)
